...
class FileSelectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("IHC - Lammoc - UFF")
        ...
        self.processamento_path = tk.StringVar()
        ...
        tk.Label(root, text="Output Folder").grid(row=6, column=0, sticky="e", padx=10, pady=5)
        ...
        tk.Entry(root, textvariable=self.processamento_path, width=50, state='readonly').grid(row=5, column=1, columnspan=2, padx=10, pady=5)
        tk.Entry(root, textvariable=self.output_path, width=50, state='readonly').grid(row=6, column=1, columnspan=2, padx=10, pady=5)

        

        # Create browse buttons
        tk.Button(root, text="Browse", command=lambda: self.browse_file(self.pits_removed_path)).grid(row=1, column=3, padx=10, pady=5)
        tk.Button(root, text="Browse", command=lambda: self.browse_file(self.weighting_factor_path)).grid(row=2, column=3, padx=10, pady=5)
        tk.Button(root, text="Browse", command=lambda: self.browse_file(self.ips_path)).grid(row=3, column=3, padx=10, pady=5)
        tk.Button(root, text="Browse", command=lambda: self.browse_file(self.q_runoff_path)).grid(row=4, column=3, padx=10, pady=5)
        tk.Button(root, text="Browse", command=self.browse_processamento_folder).grid(row=5, column=3, padx=10, pady=5)
        tk.Button(root, text="Browse", command=self.browse_output_folder).grid(row=6, column=3, padx=10, pady=5)

        # Create OK button to confirm and close the window
        tk.Button(root, text="OK", command=self.root.destroy).grid(row=7, column=1, columnspan=2, pady=10)
    ...
    def browse_processamento_folder(self):
        # Open folder dialog and update the processamento path variable
        folder_path = filedialog.askdirectory()
        self.processamento_path.set(folder_path)
...
#Saidas
endereco_processos = app.processamento_path.get()

 # This script is used to calculate the IHC Index (Zanandrea et al. 2021) using rasters as inputs.
 #    Copyright (C) 2024  Artur Cereto
...
class FileSelectorApp:
    def __init__(self, root):
        ...
        tk.Button(root, text="Open", command=lambda: self.browse_file(self.pits_removed_path, "Pits Removed DTM File")).grid(row=1, column=3, padx=10, pady=5)
        tk.Button(root, text="Open", command=lambda: self.browse_file(self.weighting_factor_path,"Weighting Factor File" )).grid(row=2, column=3, padx=10, pady=5)
        tk.Button(root, text="Open", command=lambda: self.browse_file(self.ips_path, "IPS File")).grid(row=3, column=3, padx=10, pady=5)
        tk.Button(root, text="Open", command=lambda: self.browse_file(self.q_runoff_path, "Q RunOff File")).grid(row=4, column=3, padx=10, pady=5)
        ...
        self.treshold_label = tk.Label(root, text="Threshold value:")
        self.treshold_entry = tk.Entry(root, textvariable=self.treshold, width=10, state='normal')
        self.treshold_label.grid(row=5, column=2, sticky="e", padx=10, pady=5)
        self.treshold_entry.grid(row=5, column=3, padx=10, pady=5)
        
        tk.Label(root,text="Use Drainage as Target?").grid(row=5, column=0, sticky="e", padx=10, pady=5)
...
# ## Calculos
############################################################################################################################################
# Functions to help investigate and compare results
...
print("To close this window, press enter")
print(input())
